<template>
  <div class="pa-5">
    ...
    <v-row class>
      <v-col md="3">
        <v-card class="pa-3" outlined>
          <v-card-text>
            ...
            <div class="text--primary">{{ appInfo.app_description }}</div>
            <v-switch v-model="app.active" color="success" :label="`Public`"></v-switch>
      ...
      <v-col md="9">
        <v-card outlined class="mb-3">
          <v-toolbar flat>
            <v-btn dark color="light-green" class="mr-3" depressed @click="addTable">
              <v-icon class="mr-1">grid_on</v-icon>Add Collection
            </v-btn>
            <v-btn
              v-if="app.tables.length > 1"
              value="left"
              dark
            ...
            >
              <v-icon class="mr-1">call_split</v-icon>Add Relationship
            </v-btn>
            <v-spacer></v-spacer>
            <v-btn
              dark
        ...
        <v-container fluid>
          <v-row justify="center">
            ...
            <v-expansion-panels focusable>
              <v-expansion-panel v-for="(table, i) in app.tables" :key="i" hide-actions>
                ...
                <v-expansion-panel-content>
                  ...
                  <p class="text-right pt-4 pb-0">
                    ...
                    >
                      <template v-slot:activator="{ on }">
                        <v-btn small color="red lighten-4" v-on="on" borderless>
                          Drop Table
                  ...
                  <v-toolbar flat v-for="(item, e) in table.fields" :key="i">
                    <v-row>
                      <v-col cols="12" sm="3" >
                        <v-select v-model="item.type" :items="fieldTypes" item-value="type" label="Select Type" solo outlined dense hide-details flat ></v-select>
                      </v-col>
                      <v-col cols="12" sm="6" >
                        <template v-if="item.new">
                          <v-text-field v-model="item.name" :rules="nameRules" label="Field Name *new" solo outlined dense hide-details flat required ></v-text-field>
                        </template>
                        <templat v-else>
                          <v-text-field v-model="item.name" :rules="nameRules" label="Field Name *old" solo outlined dense hide-details flat required ></v-text-field>
                        </templat>
                      </v-col>
                    </v-row>
                    ...
                    <v-toolbar-items>
                      <v-btn icon color="red" @click="removeField(i, e)"> <v-icon>mdi-delete-forever</v-icon> </v-btn>
                    </v-toolbar-items>
                  </v-toolbar>
                  ...
                  <!-- <v-list shaped>
                    ...
                    <v-list-item-group v-model="item" color="primary">
                      <v-list-item v-for="(item, e) in table.fields" :key="i">
                        <v-list-item-icon>
                          <v-select v-model="item.type" :items="fieldTypes" item-value="type" label="Select Type" solo outlined dense hide-details flat ></v-select>
                        </v-list-item-icon>
                        ...
                        <v-list-item-content>
                          <v-col cols="12" sm="12" md="10" class="pa-0">
                            <template v-if="item.new">
                              <v-text-field v-model="item.name" :rules="nameRules" md="6" label="Field Name *new" solo outlined dense hide-details flat required ></v-text-field>
                            </template>
                            <templat v-else>
                              <v-text-field v-model="item.name" :rules="nameRules" md="6" label="Field Name *old" solo outlined dense hide-details flat required ></v-text-field>
                            </templat>
                          </v-col>
                          ...
                          <v-col cols="12" sm="12" md="2" class="pa-0">
                            <p class="text-right pb-0">
                              <v-btn-toggle v-model="icon" borderless>
                                <v-btn small value="left" color="red lighten-4" @click="removeField(i, e)" > <span class="hidden-sm-and-down">Remove</span> <v-icon right>clear</v-icon> </v-btn>
                              </v-btn-toggle>
                            </p>
                          </v-col>
                        </v-list-item-content>
                  ...
                  <v-btn small dark class="ml-4 mt-3" color="light-green" @click="addField(i)">
    ...
    <v-dialog v-model="getApiDialog" width="700">
      <template v-slot:activator="{ on }"></template>

      <v-card>
        <v-card-title class="title grey lighten-2" primary-title>API - {{txtApiTable}}</v-card-title>

        <v-card-text class="pt-5">
          <v-text-field
            class="api-info-field"
            label="GET"
            prefix="GET"
            solo
            outlined
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" text @click="getApiDialog = false">Ok</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
        ...
        <v-card-text>
          <v-container>
            <v-row>
              ...
              <v-col cols="12" sm="2">
                <v-autocomplete :items="fields" label="Field" dense hide-details v-model="field"></v-autocomplete>
              </v-col>
...
export default {
  ...
  data: function() {
    return {
      ...
      item: 1,
      items: [{ text: "String", icon: "text_format" }],
      fieldTypes: [
  ...
  methods: {
    ...
    saveData() {
      ...
      for (var i = 0; i < self.app.tables.length; i++) {
        if (!self.app.tables[i].new) {
          for (var e = 0; e < self.app.tables[i].fields.length; e++) {
            if (
              self.app.tables[i].fields[e].name !=
              self.app.tables[i].fields[e].old_name
            ) {
      ...
      for (var i = 0; i < self.app.tables.length; i++) {
        if (!self.app.tables[i].new) {
          ...
          self.changedTables.push(
            JSON.parse(JSON.stringify(self.app.tables[i]))
          );
        }
      }
      ...
      if (
        ...
        if (self.table1 != self.table2) {
          ...
          if (qty > 0) {
            for (var i = 0; i < qty; i++) {
              if (
                self.app.relations[i].table1 == self.table1 &&
                self.app.relations[i].table2 == self.table2 &&
                self.app.relations[i].relationship == self.relationship
              ) {
                err = err + 1;
              }
            }
    ...
    getApiTable(i) {
      ...
      var link =
        "/api/" + this.appInfo.alias + "/" + this.app.tables[i].name + "/";
